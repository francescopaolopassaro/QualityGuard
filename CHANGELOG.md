...
* **Java gap rules**: `QG-JV-BUG-0329` catches `@Transactional` methods that call methods with an incompatible propagation mode (e.g. calling a `REQUIRES_NEW`/`NOT_SUPPORTED` method from a `REQUIRED` one), including propagation inherited from the class and upstream calls (`a.b()` where `b` is not transactional).
* **Expanded AssertJ chains** (`QG-JV-SML-0566`): detects `assertThat(s.contains(x)).isTrue()/isFalse()` → `contains(expected)`/`doesNotContain(expected)` (and `startsWith`/`endsWith`), `assertThat(x.toString()).isEqualTo(y)` → `hasToString(expectedString)`, and the `indexOf`/`compareTo` numeric-predicate simplifications (`isEqualTo(0)` → `isZero()`, `isZero()` → `startsWith`, `isEqualTo(-1)` → `doesNotContain`, …).
